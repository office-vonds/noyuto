...
unlink(__FILE__);`;
  const tmp = `/tmp/sz_tu_${postId}.php`;
  fs.writeFileSync(tmp, php);
  execSync(`curl -s --ftp-ssl --insecure -u "${FTP.user}:${FTP.pass}" -T ${tmp} "ftp://${FTP.host}/stretchzero.jp/public_html/sz_tu_${postId}.php"`, {timeout:15000});
  return new Promise(r=>{https.get(`${WP_BASE}/sz_tu_${postId}.php?t=sz2026`,res=>{let b='';res.on('data',c=>b+=c);res.on('end',()=>{try{fs.unlinkSync(tmp)}catch(e){}r(b)})}).on('error',e=>r('ERR:'+e.message))});
}

// 新しいサンクスページHTML（data属性でJS動的更新に対応）
...
<p style="margin:0 0 8px;font-size:13px" data-sz-thanks-info>ご予約いただいた店舗より、以下の電話番号からおかけいたします。</p>
<div class="sz-thanks-tel-num" data-sz-thanks-tel style="font-size:24px;font-weight:700;color:#F8742D;letter-spacing:.1em">各店舗より</div>
<p style="margin:8px 0 0;font-size:13px;color:#666">※ 知らない番号からの着信を警戒される方が増えております。<br>恐れ入りますが、スムーズにご連絡できるよう、<br>上記番号からの着信にお出になれるようお願いいたします。</p>
</div>
</div>
...
async function main() {
  console.log('=== サンクスページ更新（店舗別電話番号対応）===\n');
