...
### A-0. A-TRUCK 鈑金ページ改修 承認依頼メール送付 ⚠️最優先

**着手条件**: Gmail (office.vonds@gmail.com)
**所要**: 5-10分
**置き場**: 送信済みフォルダ確認後、KIRYUに通知

手順:
1. `seo/a-truck-repair-approval-request.md` のメール本文をコピー
2. `seo/atruck-repair-preview.html` をブラウザで開いて見た目を自分で一度確認
3. 宛先確定（A-TRUCK担当者メールアドレス）
4. プレビューHTMLを添付（またはサムにPDF化依頼）
5. 送信
6. KIRYUに「承認依頼メール送った」と一言

**何故今か**: この承認を経由しないとサム発注（鈑金デプロイ）が進まない。
クライアント信用維持の観点でも事前説明が必須。
